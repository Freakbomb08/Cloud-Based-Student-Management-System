      ...
      <div className="relative z-10 min-h-screen flex flex-col">
          ...
          <div className="container">
            <div className="max-w-xl">
              ...
              <div className="bg-surface-lowest rounded-2xl p-10 shadow-ambient">
                <h1 className="font-display font-extrabold text-4xl tracking-tight text-primary">Secure Portal Login</h1>
                <p className="mt-3 text-muted-foreground">
                  Please sign in with your institutional credentials to access student or faculty resources.
                </p>

                <form onSubmit={submit} className="mt-8 space-y-6">
                  <Field id="d-id" label="INSTITUTIONAL ID / EMAIL" icon={<User className="h-4 w-4" />} placeholder="e.g. 2024-88921" />
                  <Field
                    id="d-pwd"
                  ...
                  <div className="flex justify-end">
                    <a href="#" className="text-[11px] tracking-[0.2em] font-bold text-primary hover:underline">FORGOT SECURITY KEY?</a>
                  </div>

                  <Button type="submit" variant="primary" size="lg" className="w-full">
                    Sign In to Authority <ArrowRight />
                  </Button>
                </form>
              </div>
              ...
              <div className="mt-6 flex items-start gap-3 px-2 text-sm text-muted-foreground">
                ...
                <p className="italic">
                  Note: Portal accounts are restricted to active Faculty and Students. Access is managed by the Institutional Administration. New accounts are generated upon registration at the Registrar's Office.
                </p>
              </div>
              ...
              <p className="mt-8 text-center text-[11px] tracking-[0.25em] text-muted-foreground/70">
                © 2024 THE ACADEMIC AUTHORITY. SECURE GATEWAY V4.2
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:hidden flex-1 flex flex-col px-6 pt-10 pb-6">
          ...
          <form onSubmit={submit} className="mt-10 space-y-6">
            <Field id="m-id" label="STUDENT/FACULTY ID" icon={<User className="h-4 w-4" />} placeholder="e.g., 2024-IITM-01" />
            <Field
              id="m-pwd"
              label="PASSWORD"
              icon={<Lock className="h-4 w-4" />}
              type={showPwd ? "text" : "password"}
              placeholder="••••••••"
              labelRight={<a href="#" className="text-xs font-bold text-primary">Forgot?</a>}
              rightSlot={
          ...
          <Link to="/" className="mt-6 text-xs text-muted-foreground hover:text-primary">← Back to home</Link>
        </div>
        ...
        <footer className="lg:hidden bg-gradient-primary text-primary-foreground py-6 text-center">
          ...
          <p className="text-xs mt-2 opacity-80">© 2024 IITM Janakpuri. Editorial Information System.</p>
...
function Field({ id, label, icon, type = "text", placeholder, rightSlot, labelRight }: FieldProps) {
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <label htmlFor={id} className="text-[11px] tracking-[0.22em] font-bold text-muted-foreground">{label}</label>
        {labelRight}
      </div>
